...
public class ProducerV2{
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(ProducerV2.class);
    private static final String TOPIC = "membership-migration";

    private final KafkaProducer<String, Membership> membershipProducerV2;


    public ProducerV2() throws IOException {
        ...
        props.put(KafkaAvroSerializerConfig.USE_LATEST_WITH_METADATA, "app_version=2");


        LOGGER.info(String.format("props: %s",props));
        membershipProducerV2 = new KafkaProducer<>(props);
    }
    ...
    private void produce() {
        try {
            ...
            LOGGER.info("Sending Membership V2 record {}", membership);
            ProducerRecord<String, Membership> membershipRecord = new ProducerRecord<>(TOPIC, membership);
            membershipProducerV2.send(membershipRecord);
        } catch (SerializationException serializationException) {
            LOGGER.error("Unable to serialize Membership V2 record: {}", serializationException.getCause().getMessage());
        }
        LOGGER.info("================");
    }
    ...
    public static void main(final String[] args) throws IOException {
        ...
        LOGGER.info("Starting Membership Producer V2...");
